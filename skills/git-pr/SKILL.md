...
**모든 단계를 순서대로 수행하고, 마지막 단계까지 완료한 뒤 종료하세요.**

### Step 1. 컨텍스트 수집

스크립트를 실행해 브랜치, 커밋, diff 정보를 수집합니다.

```bash
bash scripts/generate-pr.sh [base_branch]
```

### Step 2. 변경사항 분석

커밋 메시지와 diff를 읽고 작업 의도를 파악합니다.

- 커밋 타입 감지: `feat` / `fix` / `chore`
- JIRA 이슈 번호 확인

### Step 3. PR 초안 작성

아래 PR Title Format과 PR Body Template을 사용해 초안을 작성합니다.

### Step 4. 사용자 확인

제목과 본문을 보여주고 승인을 요청합니다.

### Step 5. PR 생성

승인 후 아래 명령을 실행합니다. **반드시 `--assignee @me`와 `--label`을 포함하세요.**

```bash
gh pr create \
  --title "타입: JIRA-NUMBER, 설명" \
  --body "$(cat <<'EOF'
본문 내용
EOF
)" \
...
> 리포지토리에 해당 라벨이 없으면 `--label` 플래그를 생략합니다.

### Step 6. 완료 — PR URL 반환

PR URL을 사용자에게 보여주고 **즉시 종료합니다.**
...
**이 메시지를 출력한 후 추가 작업 없이 종료하세요. 반복하지 마세요.**
...
- **라벨 오류**: 리포지토리에 라벨이 존재하지 않으면 `--label` 플래그 생략
